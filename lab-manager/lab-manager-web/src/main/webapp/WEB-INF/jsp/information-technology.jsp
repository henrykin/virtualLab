...
<title>Virtual-Lab信息技术主界面</title>
<!-- 导入jquery核心类库 -->
...
<script type="text/javascript">
	...
	$(function() {
		...
		// 基本功能菜单加载
		$.ajax({
			url : '${pageContext.request.contextPath }/json/information.json',
			type : 'GET',
			dataType : 'text',
			...
			success : function(data) {
				...
				$.fn.zTree.init($("#treeMenu01"), setting, zNodes);
			},
			error : function(msg) {
				alert('菜单加载异常!');
			}
		...
		//用户管理目录
		$.ajax({
			url : '${pageContext.request.contextPath }/functionController_findMenu',
			type : 'GET',
			dataType : 'text',
			contentType:'application/json',
			success : function(data) {
				//alert(data);
				var zNodes = eval("(" + data + ")");
				$.fn.zTree.init($("#treeMenu02"), setting, zNodes);
			},
			error : function(msg) {
				alert('菜单加载异常!');
			}
		...
		// web聊天室菜单加载
		$.ajax({
			url : '${pageContext.request.contextPath }/json/webchat.json',
			type : 'GET',
			dataType : 'text',
			contentType:'application/json', 
			success : function(data) {
				//alert(data);
				var zNodes = eval("(" + data + ")");
				$.fn.zTree.init($("#treeMenu03"), setting, zNodes);
			},
			error : function(msg) {
				alert('菜单加载异常!');
			}
...
<body class="easyui-layout">
	...
	<div data-options="region:'west',split:true,title:'菜单导航'" style="width:200px">
		<div class="easyui-accordion" fit="true" border="false">
			<div title="信息技术实验目录" data-options="iconCls:'icon-mini-add'" style="overflow:auto">
				<ul id="treeMenu01" class="ztree"></ul>
			</div>
			<div title="用户管理目录" data-options="iconCls:'icon-mini-add'" style="overflow:auto">
				<ul id="treeMenu02" class="ztree"></ul>
			</div>
			<div title="web聊天设置" data-options="iconCls:'icon-mini-add'" style="overflow:auto">
				<ul id="treeMenu03" class="ztree"></ul>
			</div>
		</div>
	</div>
				...
				<tr>
					<td style="width: 300px;">
						<div style="color: #999; font-size: 8pt;">
							刘东峰老师实验团队研制 | Powered by <a href="http://yzw.gdut.edu.cn/info/1086/3170.htm">liudf@gdut.edu.cn</a>
						</div>
					</td>
